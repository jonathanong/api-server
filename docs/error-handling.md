...
hanging (issue #1948). The fallback sends `404 Not Found` when no headers have
been sent yet.
...
2. Sends a response using the error's `.status` property (or `404` as fallback)
   and the error's `.message` as the body.
